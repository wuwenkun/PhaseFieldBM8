...
//registerMooseObject("nucleation_bm", DiscreteNucleationMapTanh);

template <>
InputParameters
validParams<DiscreteNucleationMapTanh>()
{
  ...
  params.addClassDescription("Generates a spatial smoothed map of all nucleation sites with the "
                             ...
                             "DiscreteNucleation material. (tanh profile)");
...
DiscreteNucleationMapTanh::DiscreteNucleationMapTanh(const InputParameters & parameters)
...
void
DiscreteNucleationMapTanh::initialize()
{
...
void
DiscreteNucleationMapTanh::execute()
{
  if (_rebuild_map)
  {
...
//      }
      ...
      for (unsigned i = 0; i < _nucleus_list.size(); ++i)
      {
        ...
        val2 = 0.5*(1.0-std::tanh((r - _radius)/std::sqrt(2)/_int_width));
        value += val2;
        if (value > 1.0)
          value = 1.0;
        active_nuclei = 1;
...
void
DiscreteNucleationMapTanh::threadJoin(const UserObject & y)
{
  // if the map needs to be updated we merge the maps from all threads
  if (_rebuild_map)
  {
    const DiscreteNucleationMapTanh & uo = static_cast<const DiscreteNucleationMapTanh &>(y);
    _nucleus_map.insert(uo._nucleus_map.begin(), uo._nucleus_map.end());
  }
}

void
DiscreteNucleationMapTanh::meshChanged()
{
  _mesh_changed = true;
}

const std::vector<Real> &
DiscreteNucleationMapTanh::nuclei(const Elem * elem) const
{
